...
The experimental JSC `Attachment` accepts a foreign `JSGlobalContextRef` only
through documented unsafe entry methods. The caller guarantees that every entry
uses the same live global context, legal thread, VM lock, and host
synchronization. `Attachment` canonicalizes engine-provided callback contexts
for validation but cannot prove those host facts. It neither stores nor retains
the entry pointer and never releases the foreign context. Explicit final-entry
